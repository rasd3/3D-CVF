...
import sys
sys.path.append('/home/spalab/jhyoo/PCDet/pcdet/ops/iou3d_nms')
...
def nms_gpu(dets, nms_overlap_thresh, device_id=torch.cuda.current_device()):
    """nms in gpu. 
...
def nms_gpu_pcdet(boxes, nms_overlap_thresh, device_id=0):
    """nms in gpu. 
    ...
    num_out = iou3d_nms_cuda.nms_gpu(boxes, keep, nms_overlap_thresh)

    return order[keep[:num_out].cuda()].contiguous()

def nms_gpu_cc(dets, nms_overlap_thresh, device_id=torch.cuda.current_device()):
    boxes_num = dets.shape[0]
...
def rotate_nms_gpu(dets, nms_overlap_thresh, device_id=torch.cuda.current_device()):
...
def rotate_iou_gpu(boxes, query_boxes, device_id=torch.cuda.current_device()):
...
def rotate_iou_gpu_eval(boxes, query_boxes, criterion=-1, device_id=torch.cuda.current_device()):
    """rotated box iou running in gpu. 500x faster than cpu version
    (take 5ms in one example with numba.cuda code).
    convert from [this project](
        https://github.com/hongzhenwang/RRPN-revise/tree/master/lib/rotation).
