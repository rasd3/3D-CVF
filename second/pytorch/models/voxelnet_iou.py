        ...
        if self.training:
            ...
            if self._use_iou_branch or self._use_iou_param_partaa:
                ...
                for i in range(batch_size_dev):
                    ...
                    pos_idx = (labels[i]>0).nonzero().squeeze()
                    ...
                    box_preds_decode = self._box_coder.decode_torch(box_preds_[i][pos_idx],
                                                                    batch_anchors[i][pos_idx])
                    reg_targets_decode = self._box_coder.decode_torch(reg_targets[i][pos_idx],
                                                                      batch_anchors[i][pos_idx])
            ...
            if self._use_iou_param_partaa:
                cls_targets = cls_targets.to(torch.float) * iou_targets
            ...
            if self._use_iou_branch:
                iou_loss_reduced = iou_loss.sum() / batch_size_dev
                iou_loss_reduced *= self._iou_loss_weight
            else:
                iou_loss_reduced = iou_loss
            loss = loc_loss_reduced + cls_loss_reduced + iou_loss_reduced
            ...
            if self._multiclass_nms:
                ...
                if top_scores.shape[0] != 0:
                    ...
                    if not self._use_rotate_nms:
                        ...
                        boxes_for_nms = box_torch_ops.corner_to_standup_nd(
                            box_preds_corners)
                    if False: #self._use_nms_score_multiplied:
                        # from STD Section 3.3
                        top_ious = top_ious * top_scores
