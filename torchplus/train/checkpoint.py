...
def save(model_dir,
    ...
    with DelayedKeyboardInterrupt():
        ...
        if not ckpt_info_path.is_file():
            ...
            with open(ckpt_info_path, 'r', encoding="UTF-8") as f:
        ...
        model_state = model_state_to_cpu(model.state_dict()) if to_cpu else model.state_dict()
        ...
        if len(all_ckpts) > max_to_keep:
            ...
            os.remove(str(Path(model_dir) / Path(ckpt_to_delete).name))
...
def save_models(model_dir,
    ...
    with DelayedKeyboardInterrupt():
        name_to_model = _get_name_to_model_map(models)
        for name, model in name_to_model.items():
            save(model_dir, model, name, global_step, max_to_keep, keep_latest,to_cpu)
...
def model_state_to_cpu(model_state):
    ...
    # temp_model_state_cpu = type(model_state[key])()
    # temp_model_state = model_state[key]
    # for key2, val2 in temp_model_state.items():
    #     temp_temp_model_state_cpu = type(temp_model_state[key2])()
    #     temp_temp_model_state = temp_model_state[key2]
    #     for key3, val3 in temp_temp_model_state.items():
    #         try:
    #             temp_temp_model_state_cpu[key3] = val3.cpu()
    #         except:
    #             temp_temp_model_state_cpu[key3] = val3
    #     temp_model_state_cpu[key2] = temp_temp_model_state_cpu
    # model_state_cpu[key] = temp_model_state_cpu